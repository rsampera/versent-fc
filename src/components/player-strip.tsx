...
export function PlayerStrip({
  ...
  const canSwap = managerMode && Boolean(swapSourcePlayerId);
    ...
    <div className="rounded-[1.5rem] border border-white/10 bg-black/35 p-2.5 shadow-[0_20px_60px_rgba(0,0,0,0.25)]">
      <div className="mb-2 flex items-center justify-between gap-4 px-1.5">
        <div>
          ...
          <p className="mt-1 text-xs text-white/58 sm:text-sm">
            {managerMode
              ? "Select a player on the field, then choose a name here to swap them in or out."
      ...
      <div className="flex gap-2.5 overflow-x-auto px-1 pb-1">
        {players.map((player) => {
          ...
          const isSwapTarget = canSwap && swapSourcePlayerId !== player.id;

          return (
            <button
              key={player.id}
              className={`group min-w-[124px] rounded-[1.15rem] border px-3 py-2.5 text-left transition ${
                isActive
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <span
                  className={`rounded-full px-2 py-1 text-[0.58rem] font-black uppercase tracking-[0.18em] ${
                    isSwapTarget
                      ? "bg-[#a6ff47]/10 text-[#dfffbc] group-hover:bg-[#a6ff47]/22"
                      : isStarter
                        ? "bg-[#a6ff47]/18 text-[#dfffbc]"
                        : "bg-white/8 text-white/58"
                  }`}
                >
                  {isSwapTarget ? "Swap" : isStarter ? "Starter" : "Bench"}
                </span>
              </div>
